...
export default async (tracks: Track[]): Promise<Track[]> => {
  ...
  for(var tracksIds of listTracksIds){
    const responseListTrackIfLiked = await request({link: api.tracks_contains, method: "GET", body: {ids: tracksIds}});
    if(responseListTrackIfLiked.success){
      listTracksIdsLiked = [...listTracksIdsLiked, ...responseListTrackIfLiked.response];
    }
  }
  let returnedListTracks = tracks.map((item, index) => ({...item, isFav: listTracksIdsLiked[index]}));
