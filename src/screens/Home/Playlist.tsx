...
export default ({navigation, route}: PropsPlaylist) => {
  ...
  const init = async () => {
    ...
    if (resPlayList.success) {
      ...
      const _tracksItems = await checkIfTrackIsLiked(
        reformedPlaylist.tracks.items!,
      );
      setPlayList(prev => ({
        ...prev,
        tracks: {...prev.tracks, items: _tracksItems},
      }));
      setIsLoading(false);
    } else {
      setIsLoading(false);
  ...
  const addOrRemoveFav = ({isFav, track}: {isFav?: boolean; track: Track}) => {
    dispatch(addOrRemoveTrack({tracks, track, isFav: isFav!}));
    setPlayList(prev => {
      let indexTrack = prev.tracks.items!.findIndex(
          ...
          <View style={styles.container}>
            ...
            <View style={styles.infoContainer}>
              ...
              <View style={styles.userContainer}>
                ...
                <Text style={styles.nameOwner} numberOfLines={1}>
